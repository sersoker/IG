...
Orev::Orev(vector<GLfloat> &v,int divisiones, int modo){
	...
	creaRevolucion(v,divisiones,modo);
		...
		for(int i=0;i<v.size();i+=3){			float aux=v[i];			v[i]=v[i+1];			v[i+1]=aux;		}	
		creaRevolucion(v,divisiones,modo);
		for(int i=0;i<vertices.size();i+=3){			float aux=vertices[i];			vertices[i]=vertices[i+1];			vertices[i+1]=aux;		}	
...
void Orev::creaRevolucion2(vector<GLfloat> &v,int divisiones, int modo){
float alfa,ang,angulo;
	ang=360/divisiones;
	angulo=ang;
	int pPerfil=v.size();
	...
	for(int j=0; j<divisiones-1; j++){
		...
		for (int i=1; i <pPerfil/3; i++){
			...
			xr=vertices[(i*3)];
			xr=vertices[(i*3)+1]*cos(alfa) - (vertices[(i*3)+2])*sin(alfa);
			zr=vertices[(i*3)+1]*sin(alfa) + (vertices[(i*3)+2])*cos(alfa);
...
//Funcion que dada un numero de puntos y la cantidad de parciones calcula las caras de los vertices
void Orev::calculaCaras2(int particiones,int numeroPuntosInicial){
	//Vector auxiliar para calcular las caras;
	vector<GLint> t;
	vector <GLfloat> auxiliar;

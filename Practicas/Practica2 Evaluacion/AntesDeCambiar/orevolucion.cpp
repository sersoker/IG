...
void Orev::calculaCaras(int particiones,int numeroPuntosInicial){
	...
	int tama=particiones*numeroPuntosInicial;

	for(int i=1;i<tama;i++){
		if(i%numeroPuntosInicial!=0){
		t.push_back(i);		
		if(i+1>tama)
			t.push_back(i+1-tama);
		else
			t.push_back(i+1);
		if(i+particiones>tama)
			t.push_back(i+particiones-tama);
		else
			t.push_back(i+particiones);
		...
		if(i+particiones>tama)
			...
			t.push_back(i+particiones);
		if(i+particiones-1>tama)
		t.push_back(i+particiones-1-tama);
		else
		t.push_back(i+particiones-1);
		}
	}

	valora=numeroPuntosInicial*particiones;
	...
	t.push_back(numeroPuntosInicial*particiones+1);	t.push_back(valora);t.push_back(numeroPuntosInicial*particiones);

...
void Escena::draw_objects(int lado,char* archivo) {
	...
	float inicio=-2*M_PI;
	float fin=2*M_PI+0.001;
	float incremento=2*M_PI/8;
	...
	Orev objetoRevolucion2(v,divi,2);	
	...
	switch (figura){
		...
		break;
		case 6:
			if(modoDibujo==0)objetoRevolucion2.dibujar15(tipo);
			else objetoRevolucion2.dibujar20(tipo);
		break;	
	}
}

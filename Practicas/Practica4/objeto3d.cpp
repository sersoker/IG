...
void Objeto3d::dibujarN(int tipo){
  ...
  glTexCoordPointer(2,GL_FLOAT,0,&(texturas[0]));
  // dibujar usando vértices indexados
  glDrawElements( GL_TRIANGLES, 3*nTriangulos, GL_UNSIGNED_INT, &(triangulos[0]) );
...
void Objeto3d::cargaTextura(string nombreT){
  if(identificadorTextura==-1){
    jpg::Imagen * imagen= NULL;
    imagen = new jpg::Imagen(nombreT);
    glGenTextures(1,&identificadorTextura);
...
void Objeto3d::calculaCoorText(){
/* 
if(identificadorTextura==-1){

 int numVerti=vertices.size();
  double totald=0;

  double vectorDis[tamPerfil];
  vectorDis[0]=0;

  for(int i=1;i<tamPerfil;i++){
    totald=sqrt(  (vertices[i-1]-vertices[i])*(vertices[i-1]-vertices[i])+
            (vertices[i]-vertices[i])*(vertices[i]-vertices[i+1])+
            (vertices[i+1]-vertices[i])*(vertices[i+1]-vertices[i+2])
          );
    vectorDis[i]=vectorDis[i-1]+totald;
  }

  for(int i=0;i<numRev;i++){
    for(int j=0;j<tamPerfil;j++){
      texturas.push_back(i/numRev-1);
      texturas.push_back(vectorDis[j]/vectorDis[tamPerfil-1]);
    }
